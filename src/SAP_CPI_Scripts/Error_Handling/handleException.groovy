...
def Message processData(Message message) {
    ...
    else if (exception.getMessage().contains("mapping")) {
        messageText = "A mapping error occurred during step '${currentStep}': ${exception.getMessage()}"
        errorCode = "MAP_ERROR"
    }
    // If no specific error condition is met, use the exception message as the error message
    else {
        messageText = "An error occurred during step '${currentStep}': ${exception.getMessage()}"
        errorCode = "CPI_ERROR"
    }
